...
function AdminActivityPage() {
  ...
  return (
    <>
      ...
      <section className="panel">{filtered.length === 0 ? <EmptyState title="No activity found" message="Try changing the current filters." /> : <div className="table-scroll"><table className="ticket-table admin-activity-table"><thead><tr><th>Date</th><th>User</th><th>Action</th><th>Entity</th><th>Details</th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td>{formatLocalDate(item.timestamp)}</td><td>{item.user}</td><td>{item.actionType}</td><td>{item.entity}</td><td>{item.details}</td></tr>)}</tbody></table></div>}</section>
    </>
  )
}
