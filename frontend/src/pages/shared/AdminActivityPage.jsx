...
function relatedRecord(item, roleArea) {
  if (!item.relatedUrl) return null
  if (roleArea === 'admin') return { url: item.relatedUrl, label: item.entityType === 'User Account' ? 'View user profile' : item.entityType === 'Ticket' ? 'View ticket' : item.entityType === 'Ticket Category' ? 'View category' : 'View record' }
  if (roleArea === 'manager' && item.entityType === 'Ticket') {
    return { url: item.relatedUrl.replace('/admin/tickets/', '/manager/tickets/'), label: 'View ticket' }
  }
  return null
}

function AdminActivityPage({ roleArea = 'admin' }) {
