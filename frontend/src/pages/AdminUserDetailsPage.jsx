...
function AdminUserDetailsPage() {
  ...
  return (
    <>
      ...
      <section className="panel profile-panel"><dl className="profile-details"><div><dt>Email</dt><dd>{user.email}</dd></div><div><dt>Role</dt><dd>{user.role}</dd></div><div><dt>Department</dt><dd>{user.department}</dd></div><div><dt>Status</dt><dd>{user.status}</dd></div><div><dt>Created</dt><dd>{formatLocalDateTime(user.createdDate)}</dd></div></dl></section>
    </>
  )
}
