...
function ManagerWorkloadPage() {
  ...
  return <>
    ...
    {agents?.length > 0 && <section className="workload-grid">{agents.map((agent) => <AgentWorkloadCard agent={agent} ticketPath={`/manager/tickets?agent=${agent.userId}`} key={agent.userId} />)}</section>}
  </>
}
