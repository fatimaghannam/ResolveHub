...
internal static class ResolveHubAssistantKnowledge
{
    ...
    internal const string RolesAndNavigation = """
        ...
        Manager: view organizational tickets; filter/export reports; inspect via All Tickets -> View -> Ticket Details; request assignment through All Tickets -> Assign (Admin reviews); review IT Support Agent self-assignment requests; monitor Team Workload; review cancellation requests; report suspected duplicates for Admin review; comment publicly; view notifications and audit log. Cannot create tickets, directly assign, work IT Support Agent status transitions, or manage users/categories. Manager cannot view Private comments unless Manager is independently the ticket creator or assigned IT Support Agent, which normal Manager workflow does not establish.
    ...
    internal const string Creation = "Employees and Admins can create tickets from Create Ticket; IT Support Agents and Managers cannot. For a Manager or IT Support Agent, stop after denying creation permission and do not provide categories, fields, attachments, or creation steps. For an authorized Employee or Admin: fields are Title, Description, Category, Priority, and optional Attachments; actions are Analyze with AI, Apply Suggestions, Save as Draft, Cancel, and Submit Ticket. Submission creates an Open ticket owned by the authenticated creator. Drafts can be edited/deleted/submitted by their owner. Do not describe Report an Issue, Request a Service, or Request IT Support as controls or ticket types.";
